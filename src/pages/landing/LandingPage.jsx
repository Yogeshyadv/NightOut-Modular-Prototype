...
import { MobileAppHeader } from '../../components/common/MobileAppLayout';

export const LandingPage = ({ setRole }) => {
  return (
    <div className="fade-in" style={{ minHeight: "100vh", background: T.bg, position: "relative", overflow: "hidden" }}>
      <MobileAppHeader title="NightOut" />
      ...
      <div className="p-shrink" style={{ maxWidth: 1400, margin: "0 auto", padding: "120px 80px 60px", position: "relative", zIndex: 1 }}>
        <div className="flex-stack" style={{ display: "flex", alignItems: "center", gap: 80, flexWrap: "wrap", justifyContent: "center" }}>
          <div style={{ flex: 1, maxWidth: 560 }} className="fade-up">
            ...
            <h1 style={{ fontFamily: T.font2, fontSize: "clamp(32px, 4.5vw, 68px)", fontWeight: 700, lineHeight: 1.08, letterSpacing: "-2px", marginBottom: 20 }}>
            ...
            <div className="stack-buttons" style={{ display: "flex", gap: 12, marginBottom: 48, flexWrap: "wrap", width: '100%' }}>
              <button onClick={() => setRole("app")} className="hover-bright" style={{ padding: "14px 28px", borderRadius: 50, fontSize: 13, fontWeight: 700, background: T.green, color: "#000", border: "none", boxShadow: "0 0 30px rgba(0,200,83,0.3)", cursor: "pointer" }}>📱 User App</button>
              <button onClick={() => setRole("vendor")} className="hover-bright" style={{ padding: "14px 28px", borderRadius: 50, fontSize: 13, fontWeight: 700, background: T.glass, color: T.white, border: `1px solid ${T.border}`, cursor: "pointer", backdropFilter: "blur(10px)" }}>🏪 Vendor Dashboard</button>
              <button onClick={() => setRole("admin")} className="hover-bright" style={{ padding: "14px 28px", borderRadius: 50, fontSize: 13, fontWeight: 700, background: T.purpleGlow, color: T.purple2, border: `1px solid rgba(124,77,255,0.3)`, cursor: "pointer", backdropFilter: "blur(10px)" }}>⚡ Admin Panel</button>
            </div>
            <div className="grid-2" style={{ display: "flex", gap: 28, flexWrap: "wrap" }}>
              {[["200+", "Venues"], ["50K+", "Bookings"], ["12", "Cities"], ["3", "Roles"]].map(([n, l]) => (
                <div key={l} style={{ minWidth: 80 }}><div style={{ fontFamily: T.font2, fontSize: 24, fontWeight: 700, color: T.green, lineHeight: 1, marginBottom: 4 }}>{n}</div><div style={{ fontSize: 12, color: T.t3 }}>{l}</div></div>
              ))}
            </div>
          </div>
          <div className="phone-container" style={{ display: 'flex', justifyContent: 'center', width: '100%', maxWidth: 420, overflow: 'hidden' }}>
            <div style={{ transform: 'scale(1)', transformOrigin: 'top center', transition: 'transform 0.3s' }} className="phone-wrapper">
              <PhoneMockup />
            </div>
          </div>
        </div>
      ...
      <div className="p-shrink" style={{ maxWidth: 1400, margin: "0 auto", padding: "20px 80px 100px", position: "relative", zIndex: 1 }}>
        ...
        <div className="grid-1" style={{ display: "grid", gridTemplateColumns: "repeat(3, 1fr)", gap: 20 }}>
          {[
